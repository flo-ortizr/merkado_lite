...
export default function EditarPromocion() {
  ...
  // Cargar promoción y productos
  useEffect(() => {
    const loadPromotion = async () => {
      try {
        const promotion = await getPromotionById(promotionId);
        setFormData(promotion);
        setSelectedProducts(promotion.products.map((p: Product) => p.id_product));
      } catch (err: any) {
        setError(err.message || "Error cargando promoción");
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedData = { ...formData, product_ids: selectedProducts };
      await updatePromotion(promotionId, updatedData);
      router.push("/administrador/promociones");
    } catch (err: any) {
      setError(err.message || "Error actualizando promoción");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!formData) {
    return <div className="text-gray-300">Cargando promoción...</div>;
  }
    ...
    <div className="min-h-screen bg-gray-900 p-6 font-sans text-gray-100">
      <div className="max-w-3xl mx-auto bg-gray-800 p-8 rounded-2xl shadow-xl border border-gray-700">
        ...
        <header className="mb-6">
          <h1 className="text-3xl font-extrabold text-red-500">Editar Promoción</h1>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex gap-4 mt-6">
            ...
            >
              ...
              type="submit"
              disabled={loading}
              className="w-1/2 bg-red-600 hover:bg-red-700 text-white font-semibold py-3 rounded-lg transition disabled:opacity-50"
            >
