...
export default function GestionProveedores() {
  ...
  return (
    // LAYOUT PRINCIPAL: FLEX (Menú + Contenido)
    <div style={{ display: 'flex', width: '100%', minHeight: '100vh', backgroundColor: '#111827' }}>
        ...
        {/* --- CONTENIDO PRINCIPAL (Tu tabla y lógica original) --- */}
        <div className={`flex-1 bg-gray-900 p-4 sm:p-8 font-sans text-gray-100 ${styles.container} overflow-y-auto h-screen`}>
          <div className="max-w-7xl mx-auto">
            {/* Header */}
            <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
              <div className="flex flex-col md:flex-row md:items-center gap-4">
                {/* Nota: Borré el botón 'Home' porque ya está el Menú */}
            ...
            {loading && <div className="text-white p-4">Cargando proveedores...</div>}
            ...
            <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
              <div className="bg-gray-800 rounded-xl shadow-2xl border border-gray-700 p-6 max-w-sm w-full text-center">
                ...
                <div className="flex gap-3 justify-center">
                  <button onClick={() => setIsDeleteModalOpen(false)} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-gray-200 rounded-lg transition">Cancelar</button>
                  <button onClick={confirmDelete} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition">Sí, Eliminar</button>
                </div>
              </div>
            </div>
          )}
